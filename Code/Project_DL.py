...
valtest_Y = np.concatenate((testX[:, 1:4], testPredict), axis=1)
valtest_Y = scaler.inverse_transform(valtest_Y)

valtest_Y = valtest_Y[:,3]
...
#%%
mse = mean_squared_error(testY_actual, valtest_Y)
...
predicted = pd.DataFrame(valtest_Y, index=pd.date_range('2021-02-01',periods=12,freq='M'))
predicted.plot(color = 'Firebrick', title = 'Forecasted', legend = False)
plt.show()
#%%
